...
UPDATE_TIMES = 1  # Update m times per update
...
class Agent():
    ...
    def priority_step(self, state, action, reward, next_state, done, t_step):
        ...
        if self.tree.Size() > BATCH_SIZE * UPDATE_TIMES and t_step % UPDATE_STEP == 0:
            ...
            input_str = '\rt_step: {:d} \tafter first sample, sample_time: {:.3f} \tupdate_time: {:.3f} \tlearn_time: {:.3f} \tsum: {:.3f} \tsize: {:d}'.format(
...
if __name__ == "__main__":
    ...
    env = UnityEnvironment(
        file_name='p2_continuous-control/Reacher_Linux_NoVis/Reacher.x86_64')
