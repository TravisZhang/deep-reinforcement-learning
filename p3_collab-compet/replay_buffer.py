...
class BinaryTree:
    ...
    def InsertNode(self, key=None, input_node: BinaryNode = None, exp=None):
        ...
        # if exceed max capacity, delete root node or min node
        if self.counter > self.capacity:
            # self.DeleteNode(self.root)
            min_node = self.FindMinimum(self.root)
            self.DeleteNode(min_node)
    ...
    def FindNodeForSumUnder(self, input_sum):
        ...
        input_sum_temp = max(input_sum, 0.0)

        parent = None
        while current_node is not None:
            parent = current_node
            left_right_sum = current_node.value - current_node.key
            if input_sum_temp >= left_right_sum:
                break
            left_child_value = 0.0
            if current_node.left_child is not None:
                left_child_value = current_node.left_child.value
            if input_sum_temp < left_child_value:
                current_node = current_node.left_child
            else:
                input_sum_temp -= left_child_value
                current_node = current_node.right_child
